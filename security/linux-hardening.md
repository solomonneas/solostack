...
Replace `192.168.4.0/22` with your LAN subnet. Replace `192.168.4.86` with your host's LAN IP.
...
sudo ufw allow from 192.168.4.0/22 to any port 5173:5214 proto tcp

# Ops Deck API - LAN only
sudo ufw allow from 192.168.4.0/22 to any port 8005 proto tcp

# OpenClaw Gateway - LAN only
sudo ufw allow from 192.168.4.0/22 to any port 18789 proto tcp

# Ollama - deny from everywhere (defense-in-depth, already localhost-bound)
sudo ufw deny 11434

# mDNS - LAN only
sudo ufw allow from 192.168.4.0/22 to any port 5353 proto udp
```
...
# Should show 192.168.4.86:3389, not 0.0.0.0:3389
```
...
```
SSH: 192.168.4.86:22 only
xrdp: 192.168.4.86:3389 only
